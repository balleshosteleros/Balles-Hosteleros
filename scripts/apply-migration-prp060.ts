/**
 * Aplica la migración 20260619140000_fichajes_reavisos_hora_real_autosalida.sql (PRP-060)
 */
...
const sql = fs.readFileSync(
  path.resolve(process.cwd(), "supabase/migrations/20260619140000_fichajes_reavisos_hora_real_autosalida.sql"),
  "utf-8",
);
...
async function viaExecSqlRpc(): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    const result = (await supabase.rpc("exec_sql", { sql })) as { error: { message: string } | null };
...
async function main() {
  console.log("Aplicando migración PRP-060 (reavisos + hora real + auto-salida)...\n");
  if (accessToken) {
