/**
 * Aplica la migración 040_marketing_pagina_web.sql — CMS Página Web (PRP-029).
 * Uso: npx tsx scripts/apply-migration-040.ts
 */
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/040_marketing_pagina_web.sql",
);
const sql = fs.readFileSync(migrationPath, "utf-8");

async function main() {
  console.log("Aplicando migración 040 — CMS Página Web...\n");
  ...
  if (result.error) {
    ...
    console.log("\nArchivo: supabase/migrations/040_marketing_pagina_web.sql");
    console.log(`Tamaño: ${sql.length} bytes · ${sql.split("\n").length} líneas\n`);
    process.exit(1);
  }

  console.log("✅ Migración 040 aplicada correctamente.");
}
