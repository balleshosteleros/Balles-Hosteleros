/**
 * Aplica la migración 048_juridico_documentos_storage.sql
 *
 * Estrategia:
 *  1) Si existe SUPABASE_ACCESS_TOKEN en .env.local → Management API
 *  2) Si no, intenta exec_sql RPC
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/048_juridico_documentos_storage.sql"
);
...
async function main() {
  console.log("Aplicando migración 048 — Jurídico: bucket storage + ajustes documentos_juridicos...\n");

  if (accessToken) {
    console.log("→ Intentando vía Supabase Management API...");
    const r1 = await viaManagementApi();
    if (r1.ok) {
      console.log("✅ Migración 048 aplicada correctamente (Management API).");
      return;
    }
  ...
  if (r2.ok) {
    console.log("✅ Migración 048 aplicada correctamente (exec_sql).");
    return;
  }
  ...
  console.log(`   Archivo: supabase/migrations/048_juridico_documentos_storage.sql`);
