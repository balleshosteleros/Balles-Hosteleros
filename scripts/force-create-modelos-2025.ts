...
async function main() {
  ...
  const rows = combos.map(c => ({ empresa_id: empresaId, tipo: c.tipo, periodo: c.periodo, ejercicio, estado: "BORRADOR", casillas: {} }));
  const { data, error } = await supabase.from("modelos_aeat").insert(rows).select("id");
  console.log(error ? `✗ ${error.message}` : `✓ Creados ${data?.length} modelos 2025`);
}
main();
