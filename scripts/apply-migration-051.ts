/**
 * Aplica la migración 051_cambios_carta_calendario.sql
 */
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/051_cambios_carta_calendario.sql"
);
...
async function main() {
  console.log("Aplicando migración 051 — Calendario CAMBIOS DE CARTA...\n");

  if (accessToken) {
    console.log("→ Intentando vía Supabase Management API...");
    const r1 = await viaManagementApi();
    if (r1.ok) {
      console.log("✅ Migración 051 aplicada correctamente (Management API).");
      return;
    }
  ...
  if (r2.ok) {
    console.log("✅ Migración 051 aplicada correctamente (exec_sql).");
    return;
  }
  ...
  console.log(`   Archivo: supabase/migrations/051_cambios_carta_calendario.sql`);
  process.exit(1);
}
