/**
 * Aplica la migración 038_carta_digital.sql — Carta Digital pública (PRP-028).
 * Uso: npx tsx scripts/apply-migration-038.ts
 */
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/038_carta_digital.sql",
);
const sql = fs.readFileSync(migrationPath, "utf-8");

async function main() {
  console.log("Aplicando migración 038 — Carta Digital pública...\n");
  ...
  if (result.error) {
    ...
    console.log("\nArchivo: supabase/migrations/038_carta_digital.sql");
    console.log(`Tamaño: ${sql.length} bytes · ${sql.split("\n").length} líneas\n`);
    process.exit(1);
  }

  console.log("✅ Migración 038 aplicada correctamente.");
}
