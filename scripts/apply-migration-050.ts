/**
 * Aplica la migración 050_mi_panel_solicitudes.sql
 */
...
async function main() {
  console.log("Aplicando migración 050 — Mi Panel / Solicitudes personales...\n");

  if (accessToken) {
    console.log("→ Intentando vía Supabase Management API...");
    const r1 = await viaManagementApi();
    if (r1.ok) {
      console.log("✅ Migración 050 aplicada correctamente (Management API).");
      return;
    }
  ...
  if (r2.ok) {
    console.log("✅ Migración 050 aplicada correctamente (exec_sql).");
    return;
  }
  ...
  console.log(`   Archivo: supabase/migrations/050_mi_panel_solicitudes.sql`);
  process.exit(1);
}
