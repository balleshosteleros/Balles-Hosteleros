/**
 * Aplica la migración 041_gestoria_modelos_aeat.sql — PRP-030.
 * Uso: npx tsx scripts/apply-migration-041.ts
 */
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/041_gestoria_modelos_aeat.sql",
);
const sql = fs.readFileSync(migrationPath, "utf-8");

async function main() {
  console.log("Aplicando migración 041 — Modelos AEAT (PRP-030)...\n");
  ...
  if (result.error) {
    ...
    console.log("\nArchivo: supabase/migrations/041_gestoria_modelos_aeat.sql");
    console.log(`Tamaño: ${sql.length} bytes · ${sql.split("\n").length} líneas\n`);
    process.exit(1);
  }

  console.log("✅ Migración 041 aplicada correctamente.");
}
