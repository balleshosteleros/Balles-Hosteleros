/**
 * Aplica la migración 045_cronogramas_ejecuciones.sql — PRP-032.
 *
 * Estrategia:
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/045_cronogramas_ejecuciones.sql"
);
...
async function main() {
  console.log("Aplicando migración 045 — Cronogramas Ejecuciones (PRP-032)...\n");
  ...
  if (accessToken) {
    ...
    if (r1.ok) {
      console.log("✅ Migración 045 aplicada correctamente (Management API).");
      return;
    }
  ...
  if (r2.ok) {
    console.log("✅ Migración 045 aplicada correctamente (exec_sql).");
    return;
  }
  ...
  console.log(`   Archivo: supabase/migrations/045_cronogramas_ejecuciones.sql`);
