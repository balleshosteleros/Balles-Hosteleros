/**
 * Aplica la migración 047_profile_rol_label.sql
 *
 * Estrategia:
...
const migrationPath = path.resolve(
  process.cwd(),
  "supabase/migrations/047_profile_rol_label.sql"
);
...
async function main() {
  console.log("Aplicando migración 047 — profiles.rol_label + sync deptos↔roles...\n");

  if (accessToken) {
    console.log("→ Intentando vía Supabase Management API...");
    const r1 = await viaManagementApi();
    if (r1.ok) {
      console.log("✅ Migración 047 aplicada correctamente (Management API).");
      return;
    }
  ...
  if (r2.ok) {
    console.log("✅ Migración 047 aplicada correctamente (exec_sql).");
    return;
  }
  ...
  console.log(`   Archivo: supabase/migrations/047_profile_rol_label.sql`);
