-- ============================================================
-- 20260818200000_confirmacion_bloquea_documento_parcial.sql
-- Encargo de Iván (17-ago): un albarán al que le falta una página (SUMA Y SIGUE,
-- "pág. 1 de 2") se caza EN LA SUBIDA y, si la persona insiste en guardarlo,
-- entra marcado `documento_parcial = true` y NO se puede confirmar hasta que
-- se complete (caso Belmon Drink 15378: se cargó a medias y dos semanas después
-- la hoja ya no aparecía). Esta migración añade esa guarda al RPC de
-- confirmación; el resto de la función queda IDÉNTICA a 20260807130000.
-- La marca la quita `marcarAlbaranCompleto` (server action) cuando llegan las
