...
export function VencimientosView() {
  ...
  const filtrados = useMemo(() => datosConEstado.filter((v) => {
  ...
  const vencimientosPorDia = useMemo(() => {
    ...
    datosConEstado.forEach((v) => { if (!mapa[v.fechaVencimiento]) mapa[v.fechaVencimiento] = []; mapa[v.fechaVencimiento].push(v); });
    return mapa;
  }, [datosConEstado]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-muted flex items-center justify-center"><FileText className="h-5 w-5 text-muted-foreground" /></div><div><p className="text-2xl font-bold text-foreground">{resumen.total}</p><p className="text-xs text-muted-foreground">Total registros</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-emerald-50 flex items-center justify-center"><CheckCircle2 className="h-5 w-5 text-emerald-600" /></div><div><p className="text-2xl font-bold text-emerald-700">{resumen.alDia}</p><p className="text-xs text-muted-foreground">Al día</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-amber-50 flex items-center justify-center"><Timer className="h-5 w-5 text-amber-600" /></div><div><p className="text-2xl font-bold text-amber-700">{resumen.proximo}</p><p className="text-xs text-muted-foreground">Próximos</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-red-50 flex items-center justify-center"><AlertTriangle className="h-5 w-5 text-red-600" /></div><div><p className="text-2xl font-bold text-red-700">{resumen.vencido}</p><p className="text-xs text-muted-foreground">Vencidos</p></div></CardContent></Card>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Select value={filtroEstado} onValueChange={setFiltroEstado}>
          ...
          <SelectContent><SelectItem value="todos">Todos los estados</SelectItem>{ESTADOS_VENCIMIENTO.map((e) => <SelectItem key={e} value={e}>{e}</SelectItem>)}</SelectContent>
        </Select>
        <Select value={filtroCategoria} onValueChange={setFiltroCategoria}>
          <SelectTrigger className="w-[160px]"><SelectValue placeholder="Categoría" /></SelectTrigger>
          <SelectContent><SelectItem value="todos">Todas las categorías</SelectItem>{CATEGORIAS.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
        </Select>
      ...
      {vista === "lista" && (
        <Card>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Nombre</TableHead><TableHead>Categoría</TableHead><TableHead>Local</TableHead>
                <TableHead>Fecha</TableHead><TableHead>Frecuencia</TableHead><TableHead>Responsable</TableHead><TableHead>Estado</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtrados.length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">No se encontraron registros</TableCell></TableRow>}
              {filtrados.map((v) => (
                <TableRow key={v.id} className="cursor-pointer hover:bg-muted/50" onClick={() => { setSelected(v); setDetalleOpen(true); }}>
                  ...
                  <TableCell><Badge className={`gap-1 ${estadoColor[v.estado]}`}>{estadoIcon[v.estado]} {v.estado}</Badge></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden">
              {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map((d) => <div key={d} className="bg-muted p-2 text-center text-xs font-medium text-muted-foreground">{d}</div>)}
                  ...
                  <div key={key} className={`bg-background p-2 min-h-[80px] ${esHoy ? "ring-2 ring-primary/30 ring-inset" : ""}`}>
                    ...
                    <div className="mt-1 space-y-0.5">
                      {items.slice(0, 3).map((v) => (
                        <div key={v.id} className="flex items-center gap-1 cursor-pointer hover:opacity-80" onClick={() => { setSelected(v); setDetalleOpen(true); }}>
      ...
      <Dialog open={detalleOpen} onOpenChange={setDetalleOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          {selected && (
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">{selected.nombre}<Badge className={`${estadoColor[selected.estado]} gap-1`}>{estadoIcon[selected.estado]} {selected.estado}</Badge></DialogTitle>
              </DialogHeader>
              <Tabs defaultValue="detalle">
                ...
                <TabsContent value="detalle" className="space-y-4 mt-4">
                  ...
                  {selected.observaciones && <div><Label className="text-xs text-muted-foreground">Observaciones</Label><p className="text-sm mt-1 text-muted-foreground">{selected.observaciones}</p></div>}
                </TabsContent>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div className="col-span-2"><Label>Nombre *</Label><Input value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} placeholder="Ej: Permiso de terraza" /></div>
            <div><Label>Tipo</Label><Select value={form.tipo} onValueChange={(v) => setForm({ ...form, tipo: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{TIPOS_VENCIMIENTO.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Categoría</Label><Select value={form.categoria} onValueChange={(v) => setForm({ ...form, categoria: v as CategoriaVencimiento })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CATEGORIAS.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Local / Centro</Label><Select value={form.local} onValueChange={(v) => setForm({ ...form, local: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{LOCALES_VENCIMIENTO.map((l) => <SelectItem key={l} value={l}>{l}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div><Label>Frecuencia</Label><Select value={form.frecuencia} onValueChange={(v) => setForm({ ...form, frecuencia: v as Frecuencia })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{FRECUENCIAS.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Responsable</Label><Select value={form.responsable} onValueChange={(v) => setForm({ ...form, responsable: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{RESPONSABLES.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select></div>
            <div className="col-span-2"><Label>Proveedor / Empresa externa</Label><Input value={form.proveedorExterno} onChange={(e) => setForm({ ...form, proveedorExterno: e.target.value })} placeholder="Opcional" /></div>
            <div className="col-span-2"><Label>Observaciones</Label><Textarea value={form.observaciones} onChange={(e) => setForm({ ...form, observaciones: e.target.value })} rows={3} /></div>
          </div>
