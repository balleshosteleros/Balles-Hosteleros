...
import { DEPARTAMENTOS } from "@/data/rrhh";
import {
...
/* ================================================================
   VISTA PRINCIPAL — Listado de encuestas
   ================================================================ */
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(["todos", "activa", "borrador", "finalizada", "archivada"] as const).map((f) => (
          <Button key={f} size="sm" variant={filtro === f ? "default" : "outline"} onClick={() => setFiltro(f)}>
            {f === "todos" ? "Todos" : ESTADO_ENCUESTA_LABEL[f]}
          </Button>
        ))}
      </div>
...
/* ================================================================
   DETALLE — Editor de encuesta (4 tabs)
   ================================================================ */
...
function DetalleEncuesta({
  ...
  const addPregunta = (gIdx: number) => {
    ...
    const gs = [...enc.grupos];
    gs[gIdx] = { ...gs[gIdx], preguntas: [...gs[gIdx].preguntas, p] };
    update({ grupos: gs });
  };
  const updatePregunta = (gIdx: number, pIdx: number, partial: Partial<PreguntaEncuesta>) => {
    const gs = [...enc.grupos];
    const ps = [...gs[gIdx].preguntas];
    ps[pIdx] = { ...ps[pIdx], ...partial };
    gs[gIdx] = { ...gs[gIdx], preguntas: ps };
    update({ grupos: gs });
  };
  const removePregunta = (gIdx: number, pIdx: number) => {
    const gs = [...enc.grupos];
    gs[gIdx] = { ...gs[gIdx], preguntas: gs[gIdx].preguntas.filter((_, i) => i !== pIdx) };
    update({ grupos: gs });
  };
  ...
  const updateOpcion = (gIdx: number, pIdx: number, oIdx: number, partial: Partial<OpcionRespuesta>) => {
    const ops = [...enc.grupos[gIdx].preguntas[pIdx].opciones];
...
function ResultadosTab({ encuesta, empleados }: { encuesta: Encuesta; empleados: { id: string; nombre: string; apellidos: string }[] }) {
  ...
  const allPreguntas = encuesta.grupos.flatMap((g) => g.preguntas);
  ...
  return (
    <div className="space-y-6 mt-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card><CardContent className="pt-5 text-center"><div className="text-3xl font-bold text-foreground">{allPreguntas.length}</div><div className="text-sm text-muted-foreground">Preguntas</div></CardContent></Card>
      </div>
...
function PreguntaResultado({ pregunta, respuestas }: { pregunta: PreguntaEncuesta; respuestas: Encuesta["respuestas"] }) {
  ...
  if ((pregunta.tipo === "unica" || pregunta.tipo === "multiple") && pregunta.opciones.length > 0) {
    ...
    const config = Object.fromEntries(pregunta.opciones.map((op, i) => [op.texto || op.id, { label: op.texto, color: op.color || CHART_COLORS[i % CHART_COLORS.length] }]));
...
export default function RRHHEncuestas() {
