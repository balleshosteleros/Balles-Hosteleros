...
import { getPagosPorEmpresa, getResumenPagos, type PagoEmpleado } from "@/data/pagos";
...
import { Search, Plus, Edit2, Banknote, TrendingUp, TrendingDown, PiggyBank, HandCoins, Wallet } from "lucide-react";
...
export default function RRHHPagos() {
  ...
  // Reload data when empresa changes
  ...
  const pagosFiltrados = useMemo(() => {
    ...
    if (busqueda) {
      ...
      resultado = resultado.filter(p => p.empleadoNombre.toLowerCase().includes(q));
    }
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Control de pagos en efectivo</CardTitle>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/40">
                  ...
                  <TableHead className="text-right">H.R</TableHead>
                  ...
                  <TableHead className="text-right">Prop. Mant.</TableHead>
                  <TableHead className="text-right font-bold">Total</TableHead>
                  <TableHead className="text-center w-[80px]">Pagado</TableHead>
              ...
              <TableBody>
                {pagosFiltrados.map(p => (
                  <TableRow key={p.id} className={p.pagado ? "bg-emerald-50/40 dark:bg-emerald-950/10" : ""}>
                    ...
                    <TableCell className="text-center">
                      {p.fijo ? <Badge variant="secondary" className="text-[10px]">FIJO</Badge> : <span className="text-muted-foreground text-xs">—</span>}
                    </TableCell>
                    ...
                    <TableCell className="text-center">
                      <Checkbox checked={p.pagado} onCheckedChange={() => togglePagado(p.id)} />
                    </TableCell>
                    <TableCell>
                ...
                <TableRow className="bg-muted/60 font-semibold border-t-2">
                  ...
                  <TableCell className="text-center">
                    <Badge variant={pagos.every(p => p.pagado) ? "default" : "secondary"} className="text-[10px]">
                      {pagos.filter(p => p.pagado).length}/{pagos.length}
                    </Badge>
                  </TableCell>
                  <TableCell />
                </TableRow>
      ...
      <Dialog open={!!editando} onOpenChange={open => !open && setEditando(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Editar pago — {editando?.empleadoNombre}</DialogTitle>
          </DialogHeader>
...
function EditForm({ pago, onSave }: { pago: PagoEmpleado; onSave: (d: Partial<PagoEmpleado>) => void }) {
  ...
  const campos: { key: keyof PagoEmpleado; label: string }[] = [
    ...
    { key: "propinaMantenimiento", label: "Propina Mant." },
  ];
