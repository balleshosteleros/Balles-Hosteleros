...
import {
  Film, Camera, FileText, ImageIcon, Clock, Plus, Search, FolderOpen,
  ExternalLink, Trash2, Edit, Video, BookOpen,
} from "lucide-react";
...
function buildSampleData(empresaId: string): ContenidoItem[] {
  const base: Omit<ContenidoItem, "id" | "empresaId">[] = [
    ...
    { tipo: "grabacion", titulo: "Grabación cocina en vivo", descripcion: "Sesión de grabación del proceso de elaboración del plato estrella", estado: "en_produccion", responsable: "Carlos", fecha: "2026-04-03", enlaceExterno: "https://drive.google.com/ejemplo", etiquetas: ["cocina", "making-of"] },
    ...
    { tipo: "reel", titulo: "Reel plato del día", descripcion: "Reel corto mostrando la presentación del plato del día", estado: "publicado", responsable: "María López", fecha: "2026-04-05", enlaceExterno: "https://instagram.com/reel/ejemplo", etiquetas: ["foodie", "reel"] },
...
export default function MarketingContenido() {
  ...
  const counts = SECCIONES.map((s) => ({
    ...s,
    count: items.filter((i) => i.tipo === s.value).length,
  }));
  ...
  function handleSave(item: ContenidoItem) {
    ...
    allData[empresaActual.id] = items;
    setModalOpen(false);
    setEditItem(null);
  }
...
function ContenidoModal({ open, onClose, onSave, item, tipo, empresaId }: {
  ...
  if (item !== lastItem) {
    ...
    setEstado(item?.estado ?? "borrador");
    setResponsable(item?.responsable ?? "");
    setEnlace(item?.enlaceExterno ?? "");
    setEtiquetas(item?.etiquetas.join(", ") ?? "");
    setLastItem(item);
  }
  ...
  function handleSubmit() {
    ...
    onSave({
      ...
      tipo: item?.tipo ?? tipo,
      titulo, descripcion, estado, responsable,
      fecha: item?.fecha ?? now,
      enlaceExterno: enlace,
