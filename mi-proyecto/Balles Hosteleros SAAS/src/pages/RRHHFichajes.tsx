...
import { getFichajesPorEmpresa, getIncidenciasPorEmpresa, getConfigFichajes, ESTADO_FICHAJE_LABEL, ESTADO_FICHAJE_COLOR, TIPOS_INCIDENCIA_LABEL } from "@/data/fichajes";
import type { EstadoFichaje, Fichaje, ConfigFichajes } from "@/data/fichajes";
...
export default function RRHHFichajes() {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="pt-4 pb-4 text-center">
          ...
          <p className="text-2xl font-bold">{kpis.completos}</p>
          <p className="text-xs text-muted-foreground">Completos / Validados</p>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-4 text-center">
          <AlertTriangle className="mx-auto h-5 w-5 text-destructive mb-1" />
          <p className="text-2xl font-bold">{kpis.conIncidencia}</p>
          <p className="text-xs text-muted-foreground">Con incidencias</p>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-4 text-center">
          <Clock className="mx-auto h-5 w-5 text-sky-500 mb-1" />
          <p className="text-2xl font-bold">{kpis.horasTotales.toFixed(1)}h</p>
      ...
      <Tabs defaultValue="fichajes" className="space-y-4">
              ...
              <TableBody>
                {fichajesFiltrados.map(f => (
                  <TableRow key={f.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setFichajeModal(f)}>
                    <TableCell>
                      <div>
                        <p className="font-medium text-sm">{f.empleadoNombre}</p>
                    ...
                    <TableCell className="text-sm font-mono">
                      {f.pausaInicio && f.pausaFin ? `${f.pausaInicio}-${f.pausaFin}` : "—"}
                    </TableCell>
        ...
        <TabsContent value="historial" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Registro completo ordenado cronológicamente</CardDescription>
            </CardHeader>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {incidencias.map(i => (
                  <TableRow key={i.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-xs">{TIPOS_INCIDENCIA_LABEL[i.tipo]}</Badge>
                    </TableCell>
        ...
        <TabsContent value="config" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Configuración de fichajes</CardTitle>
              <CardDescription>Ajustes generales del sistema de fichajes</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <Label className="font-medium">Permitir fichaje manual</Label>
                  <p className="text-xs text-muted-foreground">Los empleados pueden registrar fichajes manualmente</p>
                </div>
                <Switch checked={config.permitirManual} onCheckedChange={v => setConfig(c => ({ ...c, permitirManual: v }))} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <Label className="font-medium">Requiere validación</Label>
                  <p className="text-xs text-muted-foreground">Los fichajes deben ser validados por un responsable</p>
                </div>
                <Switch checked={config.requiereValidacion} onCheckedChange={v => setConfig(c => ({ ...c, requiereValidacion: v }))} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <Label className="font-medium">Pausas activas</Label>
                  <p className="text-xs text-muted-foreground">Permitir registrar pausas dentro del fichaje</p>
                </div>
                <Switch checked={config.pausasActivas} onCheckedChange={v => setConfig(c => ({ ...c, pausasActivas: v }))} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <Label className="font-medium">Tolerancia horaria (minutos)</Label>
                  <p className="text-xs text-muted-foreground">Margen antes de generar incidencia por desfase</p>
                </div>
        ...
        <DialogContent>
          ...
          {fichajeModal && (
            <div className="space-y-3 text-sm">
              ...
              {fichajeModal.incidencia && (
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3">
                  <p className="text-sm font-medium text-destructive">{fichajeModal.incidencia}</p>
                </div>
              )}
              {fichajeModal.observaciones && (
                <div><span className="text-muted-foreground">Observaciones:</span><p>{fichajeModal.observaciones}</p></div>
              )}
              {fichajeModal.validadoPor && (
                <div><span className="text-muted-foreground">Validado por:</span><p className="font-medium">{fichajeModal.validadoPor}</p></div>
              )}
            </div>
          )}
          <DialogFooter>
