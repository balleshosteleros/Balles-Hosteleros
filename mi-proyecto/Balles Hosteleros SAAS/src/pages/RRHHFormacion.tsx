...
import { getResumenFormacion, getProgresoPorEmpresa, getEstadisticasMensuales, PORTAL_FORMATIVO_URL } from "@/data/formacion";
...
import {
  GraduationCap, ExternalLink, Users, Award, Video, TrendingUp,
  BookOpen, CheckCircle2, XCircle, BarChart3, Target, Clock,
} from "lucide-react";
...
export default function RRHHFormacion() {
  ...
  const chartConfig = {
    ...
    evaluacionesRealizadas: { label: "Evaluaciones", color: "hsl(var(--chart-3))" },
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Panel de control y seguimiento del portal formativo externo
            </p>
          </div>
        </div>
        <Button
          onClick={() => window.open(`${PORTAL_FORMATIVO_URL}/${empresaActual.id}`, "_blank")}
          className="gap-2"
        >
          <ExternalLink className="h-4 w-4" />
          Acceder al portal formativo
        </Button>
      </div>
      ...
      <Card className="border-primary/20 bg-primary/5">
        <CardContent className="py-3 px-4 flex items-center gap-3">
          ...
          <p className="text-sm text-foreground">
            El portal formativo es un <strong>software independiente</strong> conectado al SaaS principal.
            Los empleados acceden con las mismas credenciales. Toda la actividad se reporta aquí automáticamente.
          </p>
        </CardContent>
      </Card>
      ...
      {/* Row: Avance medio + Tasa aprobado pie */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Progreso medio */}
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <TrendingUp className="h-4 w-4" /> Avance medio global
            </CardTitle>
          </CardHeader>
        ...
        {/* Pie aprobados/suspendidos */}
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground">Aprobados vs Suspendidos</CardTitle>
          </CardHeader>
          <CardContent className="flex items-center justify-center">
            ...
            <div className="ml-4 space-y-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground">Aprobadas: {resumen.aprobadas}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="h-3 w-3 rounded-full bg-destructive" />
                <span className="text-foreground">Suspendidas: {resumen.suspendidas}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground">Desglose de vídeos</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-muted-foreground">Genéricos</span>
                <span className="font-semibold text-foreground">{resumen.videosGenericos}</span>
              </div>
              <Progress value={(resumen.videosGenericos / Math.max(resumen.totalVideos, 1)) * 100} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-muted-foreground">Específicos por puesto</span>
                <span className="font-semibold text-foreground">{resumen.videosEspecificos}</span>
              </div>
              <Progress value={(resumen.videosEspecificos / Math.max(resumen.totalVideos, 1)) * 100} className="h-2" />
            </div>
            <p className="text-xs text-muted-foreground">Total: {resumen.totalVideos} vídeos subidos</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground">Evolución mensual de empleados formados</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground">Estado de formación por puesto</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="border bg-card">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
            <BarChart3 className="h-4 w-4" /> Métricas por puesto
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">En formación</TableHead>
                ...
                <TableHead className="text-center">Avance medio</TableHead>
                <TableHead className="text-center">% Finalización</TableHead>
            ...
            <TableBody>
              {resumen.porPuesto.map((p) => (
                <TableRow key={p.puesto}>
                  <TableCell className="font-medium text-foreground">{p.puesto}</TableCell>
      ...
      <Card className="border bg-card">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
            <Users className="h-4 w-4" /> Seguimiento individual de empleados
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {progresos.map((p) => {
                ...
                return (
                  <TableRow key={p.id}>
                    <TableCell className="font-medium text-foreground">{p.empleadoNombre}</TableCell>
      ...
      {/* Evolución notas medias */}
      <Card className="border bg-card">
        <CardHeader>
          <CardTitle className="text-sm font-medium text-muted-foreground">Evolución de notas medias</CardTitle>
        </CardHeader>
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[220px] w-full">
            <LineChart data={stats}>
              ...
              <ChartTooltip content={<ChartTooltipContent />} />
              <Line type="monotone" dataKey="notaMedia" stroke="var(--color-notaMedia)" strokeWidth={2} dot={{ r: 4 }} />
            </LineChart>
          </ChartContainer>
        </CardContent>
      </Card>
