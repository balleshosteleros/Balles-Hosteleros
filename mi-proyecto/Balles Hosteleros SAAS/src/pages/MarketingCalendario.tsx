...
import { CalendarioView } from "@/components/marketing/CalendarioView";
import { PublicacionModal } from "@/components/marketing/PublicacionModal";
import { DetallePanel } from "@/components/marketing/DetallePanel";
import { CuentasConectadas } from "@/components/marketing/CuentasConectadas";
...
export default function MarketingCalendario({ embedded }: { embedded?: boolean } = {}) {
  ...
  const handleSelect = (item: ItemCalendario) => {
    setDetalleItem(item);
    setDetalleOpen(true);
  };
    ...
    <div className={embedded ? "space-y-5" : "p-4 md:p-6 space-y-5"}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card><CardContent className="p-4 text-center">
          ...
          <p className="text-xs text-muted-foreground">Total elementos</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold text-foreground">{pubCount}</p>
          <p className="text-xs text-muted-foreground">Publicaciones</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold text-foreground">{evtCount}</p>
          <p className="text-xs text-muted-foreground">Eventos</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold text-primary">{progCount}</p>
              ...
              <div className="space-y-2">
                {items.filter((i) => i.tipo === "publicacion").length === 0 ? (
                  ...
                  items.filter((i) => i.tipo === "publicacion").map((it) => {
                    const pub = it as import("@/data/marketing").Publicacion;
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-xs mt-1">Cuando las cuentas estén conectadas, aquí aparecerán métricas de rendimiento de publicaciones.</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
