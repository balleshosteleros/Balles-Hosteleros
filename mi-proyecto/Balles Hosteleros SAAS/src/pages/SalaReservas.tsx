...
import {
  Plus, Search, Users, ChevronLeft, ChevronRight, Clock, Eye, ListPlus, X,
} from "lucide-react";
import {
  SAMPLE_MESAS, SAMPLE_RESERVAS, SAMPLE_LISTA_ESPERA,
  Mesa, Reserva, ListaEspera,
  EstadoReserva, ZonaSala, TurnoReserva,
...
/* ===================== MAIN COMPONENT ===================== */
export default function SalaReservas() {
  ...
  const reservasDia = useMemo(() =>
    reservas.filter(r => r.fecha === fecha), [reservas, fecha]);

  const reservasTurno = useMemo(() =>
    turno === "DIA_COMPLETO" ? reservasDia : reservasDia.filter(r => r.turno === turno),
    [reservasDia, turno]);
  ...
  const mesasLibres = mesasActivas.length - mesasOcupadas;

  /* mesa effective state for the current turn */
  const getMesaEstadoTurno = (m: Mesa): string => {
    const reservasMesa = reservasTurno.filter(r => r.mesaId === m.id && !["CANCELADA", "NO SHOW", "COMPLETADA"].includes(r.estado));
    if (reservasMesa.length === 0) return "LIBRE";
    const walkIn = reservasMesa.find(r => r.estado === "WALK_IN");
    if (walkIn) return "OCUPADA";
    return "RESERVADA";
  };
  ...
  const turnoLabel = turno === "DIA_COMPLETO" ? "Día completo" : turno === "COMIDA" ? "Comida" : "Cena";
  const turnoActivo = turno !== "DIA_COMPLETO" && turno === "CENA"; // simple toggle
      ...
      <div className="shrink-0 border-b bg-card px-4 py-2 flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1">
          {(["COMIDA", "CENA", "DIA_COMPLETO"] as const).map(t => (
            <Button key={t} size="sm" variant={turno === t ? "default" : "outline"}
              className={cn("text-xs h-8", turno === t && "font-bold")}
              onClick={() => setTurno(t)}>
        ...
        <div className="w-[420px] shrink-0 border-r flex flex-col bg-card overflow-hidden">
          ...
          <div className="p-3 border-b space-y-2">
            ...
            <div className="relative">
              ...
              <Input placeholder="Buscar por nombre, apellido, teléfono..." className="pl-8 h-8 text-xs" value={busqueda} onChange={e => setBusqueda(e.target.value)} />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {reservasFiltradas.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-8">Sin reservas para este turno</p>
            )}
            {reservasFiltradas.map(r => {
              ...
              return (
                <button key={r.id}
                  ...
                  className={cn(
                    "w-full grid grid-cols-[50px_50px_1fr_40px_70px_90px] gap-1 px-3 py-2.5 text-xs border-b hover:bg-muted/40 text-left transition-colors",
                    selectedReserva?.id === r.id && "bg-primary/10"
                  )}>
                  ...
                  return (
                    <Popover key={m.id}>
                      <PopoverTrigger asChild>
                        <button
                          className={cn(
                            "absolute rounded-lg flex flex-col items-center justify-center text-white text-[10px] font-bold shadow-md border border-white/10 transition-all cursor-pointer",
                            bgClass,
                            selectedMesa?.id === m.id && "ring-2 ring-primary"
                          )}
                          style={{
                            left: `${m.x}%`, top: `${m.y}%`,
                            width: `${m.ancho}%`, height: `${m.alto}%`,
                        ...
                        >
                          ...
                          {firstR && (
                            <span className="text-[8px] font-normal mt-0.5 opacity-90 truncate max-w-full px-1">
                              {firstR.hora}{" "}{isWalkIn ? "WALK IN" : firstR.cliente}
                            </span>
                          )}
                          {firstR && firstR.apellidos && !isWalkIn && (
                            <span className="text-[8px] font-normal opacity-70 truncate max-w-full px-1">{firstR.apellidos}</span>
                          )}
                        </button>
                      </PopoverTrigger>
                      <PopoverContent className="w-72 p-3">
                        <MesaPopover mesa={m} reservas={rs} onAsignar={() => {
                          setSelectedMesa(m);
                          setShowNueva(true);
...
function NuevaReservaForm({ fecha, turno, onClose, onSave }: {
  ...
  const [form, setForm] = useState({
    ...
    fecha, hora: "", turno,
    comensales: 2, zona: "" as ZonaSala | "", observaciones: "",
    esWalkIn: false,
  });
