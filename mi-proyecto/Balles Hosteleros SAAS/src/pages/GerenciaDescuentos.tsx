...
import { Descuento, ResultadoMensual, ResultadoDato, buildDefaultDescuentos, buildDefaultResultados } from "@/data/descuentos";
...
// Per-company state stored outside React for simplicity (mirrors EmpresaContext pattern)
const allDescuentos: Record<string, Descuento[]> = {};
const allResultados: Record<string, ResultadoMensual[]> = {};

function getDescuentos(empresaId: string) {
  if (!allDescuentos[empresaId]) allDescuentos[empresaId] = buildDefaultDescuentos();
  return allDescuentos[empresaId];
}
function getResultados(empresaId: string) {
  if (!allResultados[empresaId]) allResultados[empresaId] = buildDefaultResultados();
  return allResultados[empresaId];
}
...
export default function GerenciaDescuentos() {
  ...
  const filtrados = useMemo(() => {
    ...
    if (busqueda.trim()) {
      ...
      list = list.filter((d) => d.codigo.toLowerCase().includes(q) || d.ejecucion.toLowerCase().includes(q));
    }
    return list;
  ...
  function openCrear() {
    setEditando(null);
    setForm({ codigo: "", ejecucion: "", tenerEnCuenta: "", activo: true, observaciones: "" });
    setModalOpen(true);
  }
  function openEditar(d: Descuento) {
    setEditando(d);
    setForm({ ...d });
    setModalOpen(true);
  }
  function guardar() {
    ...
    if (editando) {
      ...
      allDescuentos[empresaActual.id] = updated;
      toast.success("Descuento actualizado");
    } else {
      const nuevo: Descuento = {
        id: `d-${Date.now()}`, codigo: form.codigo!, ejecucion: form.ejecucion || "",
        tenerEnCuenta: form.tenerEnCuenta || "", activo: form.activo ?? true,
        observaciones: form.observaciones || "", creadoPor: "Usuario",
  ...
  // Resultados charts
  const chartMensualTotal = useMemo(() => {
    return resultados.map((r) => ({
      mes: r.etiqueta,
      total: r.datos.reduce((s, d) => s + d.totalDescontado, 0),
      usos: r.datos.reduce((s, d) => s + d.usos, 0),
    }));
  }, [resultados]);
  ...
  const pieData = useMemo(() => {
    if (!ultimoMes) return [];
    return ultimoMes.datos.map((d) => ({ name: d.codigoDescuento, value: d.totalDescontado }));
  }, [ultimoMes]);

  const chartConfig = {
    total: { label: "Total descontado (€)", color: "hsl(var(--primary))" },
    usos: { label: "Nº de usos", color: "hsl(210 60% 55%)" },
  };
            ...
            <Card>
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {filtrados.length === 0 && (
                      <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">No se encontraron descuentos</TableCell></TableRow>
                    )}
                    {filtrados.map((d) => (
                      <TableRow key={d.id}>
          ...
          <TabsContent value="resultados" className="space-y-6">
            <Card>
              <CardHeader className="flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="gap-2" onClick={() => toast.info("Funcionalidad de lectura de PDF preparada para integración con backend")}><Upload className="h-4 w-4" /> Subir PDF</Button>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground">Sube el PDF exportado del software del restaurante para extraer automáticamente los datos de descuentos del mes y generar las gráficas.</p>
              </CardContent>
            </Card>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card>
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total descontado (último mes)</CardTitle></CardHeader>
                <CardContent><p className="text-2xl font-bold text-foreground">{ultimoMes ? `${ultimoMes.datos.reduce((s, d) => s + d.totalDescontado, 0).toLocaleString("es-ES")} €` : "—"}</p></CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Usos totales (último mes)</CardTitle></CardHeader>
                <CardContent><p className="text-2xl font-bold text-foreground">{ultimoMes ? ultimoMes.datos.reduce((s, d) => s + d.usos, 0).toLocaleString("es-ES") : "—"}</p></CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Tipos de descuento usados</CardTitle></CardHeader>
                <CardContent><p className="text-2xl font-bold text-foreground">{ultimoMes ? ultimoMes.datos.length : "—"}</p></CardContent>
              </Card>
            </div>
            ...
            <Card>
              ...
              <CardContent>
                <ChartContainer config={chartConfig} className="h-[300px] w-full">
                  <BarChart data={chartMensualTotal}>
            ...
            <Card>
              ...
              <CardContent>
                <ChartContainer config={chartConfig} className="h-[300px] w-full">
                  <LineChart data={chartMensualTotal}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" />
                    <XAxis dataKey="mes" className="text-xs" />
                    <YAxis className="text-xs" />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Line type="monotone" dataKey="usos" stroke="hsl(210 60% 55%)" strokeWidth={2} dot={{ r: 4 }} />
                  </LineChart>
                </ChartContainer>
              </CardContent>
            </Card>

            {/* Pie chart — distribución último mes */}
            {pieData.length > 0 && (
              <Card>
                ...
                <CardContent className="flex justify-center">
                  <div className="h-[320px] w-full max-w-lg">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={110} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={11}>
                  ...
                  <TableBody>
                    {resultados.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-right">{r.datos.reduce((s, d) => s + d.totalDescontado, 0).toLocaleString("es-ES")} €</TableCell>
                        <TableCell className="text-right">{r.datos.reduce((s, d) => s + d.usos, 0)}</TableCell>
                        <TableCell className="text-right">{r.datos.length}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>Código *</Label>
                <Input value={form.codigo ?? ""} onChange={(e) => setForm({ ...form, codigo: e.target.value })} placeholder="Ej: CHUPITO" />
              </div>
              <div className="space-y-2">
                <Label>Ejecución</Label>
                <Textarea rows={3} value={form.ejecucion ?? ""} onChange={(e) => setForm({ ...form, ejecucion: e.target.value })} placeholder="Descripción operativa…" />
              </div>
              <div className="space-y-2">
                <Label>A tener en cuenta</Label>
                <Textarea rows={3} value={form.tenerEnCuenta ?? ""} onChange={(e) => setForm({ ...form, tenerEnCuenta: e.target.value })} placeholder="Condiciones y reglas…" />
              </div>
              <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <Label>Activo</Label>
                <Switch checked={form.activo ?? true} onCheckedChange={(v) => setForm({ ...form, activo: v })} />
              </div>
            </div>
            <DialogFooter>
            ...
            {detalle && (
              <div className="space-y-4">
                ...
                <div className="space-y-3 text-sm">
                  <div><p className="font-medium text-foreground mb-1">Ejecución</p><p className="text-muted-foreground">{detalle.ejecucion || "—"}</p></div>
                  <div><p className="font-medium text-foreground mb-1">A tener en cuenta</p><p className="text-muted-foreground">{detalle.tenerEnCuenta || "—"}</p></div>
                  <div><p className="font-medium text-foreground mb-1">Observaciones internas</p><p className="text-muted-foreground">{detalle.observaciones || "—"}</p></div>
