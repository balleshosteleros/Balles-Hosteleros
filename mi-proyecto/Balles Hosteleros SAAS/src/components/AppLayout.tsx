...
const ROUTE_TITLES: Record<string, string> = {
  ...
  "/rrhh/empleados/:id": "FICHA EMPLEADO",
...
export function AppLayout({ children }: { children?: React.ReactNode }) {
  ...
  if (!title && location.pathname.startsWith("/rrhh/empleados/")) title = "FICHA EMPLEADO";

  const rolLabel = roles.length > 0 ? roles[0].charAt(0).toUpperCase() + roles[0].slice(1) : "—";
      ...
      <div className="min-h-screen flex w-full">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 flex items-center border-b bg-card px-4 shrink-0 gap-3">
            ...
            {title && <h1 className="text-sm font-bold tracking-wide text-foreground">{title}</h1>}
              ...
              {user && (
                <>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <Badge variant="secondary" className="text-[10px]">{rolLabel}</Badge>
                  </div>
