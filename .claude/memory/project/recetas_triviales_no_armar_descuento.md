...
3. **Ágora NO exporta las recetas por su API** (verificado a fondo 23-jun): `export-master` no tiene filtro de recetas/escandallos (lista completa: Products, Menus, Families, Stocks, Suppliers…); `filter=Menus` devuelve combos de productos de venta (`MenuGroups`→`Products`), NO ingredientes de materia prima. La ÚNICA vía API sería `POST /api/custom-query` con un **`QueryGuid`** de una **consulta personalizada creada en el back-office de Ágora** ("Guía de informes personalizados") que liste producto→componentes+cantidad; sin ese GUID válido responde **400**, y no hay endpoint para descubrir las consultas existentes. **→ Acción recurrente recomendada: pedir a Ágora que cree ese informe de escandallos y nos pase el `QueryGuid`**; con él se sacarían todas las recetas vía custom-query de forma automática. (Nota: Ágora sí conoce los ingredientes internamente — los expone como `RemovedIngredients` en las líneas de venta — pero no la receta base completa.)
