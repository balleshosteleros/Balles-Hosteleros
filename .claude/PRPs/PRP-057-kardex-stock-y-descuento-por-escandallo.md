...
### 2026-06-16: Conexión Ágora visible/editable en el producto + cómo entran los menús
- **Vista nueva** `ConexionAgoraSection` (Card en `ProductoDetalle` de venta) + `actions/conexion-agora-actions.ts`: muestra ID de Ágora (editable → "configurable desde Balles"), **nombre con que Ágora lo vende** (derivado de `pos_ticket_lineas`, no de una llamada a Ágora) y si se vende + unidades.
