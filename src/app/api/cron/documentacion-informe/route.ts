 ...
 *
 * Cuándo se vio a cada uno por última vez NO se calcula aquí: lo hace
 * `senal-de-uso.ts`, y ahí está explicado por qué la lectura no lleva recorte de
 * fecha. Resumen de la trampa, porque costó un correo acusando de no abrir la
 * app a siete personas que fichaban esa semana: **quien lee no recorta, quien
 * compara no consulta**. `AVISO_DESDE` solo se compara; jamás entra en un
 * `.gte()`.
 *
...
import { clasificarUso, leerUltimaSenal } from "@/features/primer-acceso/lib/senal-de-uso";
...
export async function GET(request: Request) {
  ...
  // Cuándo se vio a cada uno por última vez. Sin recorte de fecha: ver
  // `senal-de-uso.ts`. Un `null` de aquí significa «no ha entrado nunca», y por
  // eso se puede escribir así en el correo.
  const senales = await leerUltimaSenal(
    supabase,
    [...porPersona.values()].map((p) => p.userId).filter((id): id is string => Boolean(id)),
  );
  ...
  for (const p of porPersona.values()) {
    ...
    const senal = p.userId ? senales.get(p.userId) ?? null : null;
    const { estado, frase } = clasificarUso(senal, Boolean(p.userId), AVISO_DESDE);
    const detalle = `falta ${pendientes.map((c) => c.etiqueta).join(", ")} · ${frase}`;

    if (estado === "ha-visto-el-aviso") vistoSinHacer.push(fila(p.nombre, empresas, detalle));
    else sinEntrar.push(fila(p.nombre, empresas, detalle));
  }
