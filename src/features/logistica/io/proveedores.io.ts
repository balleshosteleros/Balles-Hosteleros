...
const proveedorIOSchema = z.object({
  ...
  telefonoComercial: z.string().nullable().optional(),
  emailPrincipal: z.string().nullable().optional(),
  emailComercial: z.string().nullable().optional(),
  emailPedidos: z.string().nullable().optional(),
  emailContabilidad: z.string().nullable().optional(),
...
export const proveedoresIO: ModuleIO<ProveedorImport> = {
  ...
  columns: [
    ...
    { key: "personaContacto", label: "Comercial", aliases: ["contacto", "persona de contacto"], example: "Juan García" },
    { key: "telefonoPrincipal", label: "Teléfono empresa", aliases: ["telefono", "telefono principal"], example: "912345678" },
    { key: "telefonoSecundario", label: "Teléfono secundario" },
    { key: "telefonoComercial", label: "Teléfono comercial", aliases: ["telefono comercial"] },
    { key: "emailPrincipal", label: "Email empresa", aliases: ["email", "email principal"], example: "ventas@marisol.es" },
    { key: "emailComercial", label: "Email comercial", aliases: ["correo comercial"] },
    { key: "emailPedidos", label: "Email pedidos" },
    { key: "emailContabilidad", label: "Email contabilidad", aliases: ["correo contabilidad", "email incidencias", "correo incidencias"] },
  ...
  fetchAll: async () => {
    ...
    return rows.map<ProveedorImport>((r) => ({
      ...
      telefonoComercial: r.telefono_comercial ?? null,
      emailPrincipal: r.email_principal ?? null,
      emailComercial: r.email_comercial ?? null,
      emailPedidos: r.email_pedidos ?? null,
      emailContabilidad: r.email_contabilidad ?? null,
      web: r.web ?? null,
