...
import {
  ...
  updateRecetaEstado,
  deleteNuevaReceta,
} from "@/features/cocina/actions/nuevas-recetas-actions";
...
function progreso(pasos: NuevaReceta["pasos"]): number {
...
export function NuevasRecetasView() {
  const [recetas, setRecetas] = useState<NuevaReceta[]>([]);
  ...
  const [detalle, setDetalle] = useState<NuevaReceta | null>(null);

  function mapDbReceta(r: Record<string, unknown>): NuevaReceta {
    return {
      id: r.id as string,
  ...
  const cargarRecetas = useCallback(async () => {
    try {
      setCargando(true);
      const res = await listNuevasRecetas();
      if (res.ok) {
        setRecetas((res.data as Record<string, unknown>[]).map(mapDbReceta));
      }
    } catch {
  ...
  async function crear() {
    ...
    try {
      const res = await createNuevaReceta({
        nombre: form.nombre.trim(),
        descripcion: form.descripcion.trim(),
        destino: form.destino,
      });
      if (!res.ok) { toast.error(res.error ?? "Error al crear receta"); return; }
      setShowNew(false);
  ...
  const PASO_TO_DB: Record<keyof NuevaReceta["pasos"], string> = {
  ...
  async function togglePaso(recetaId: string, paso: keyof NuevaReceta["pasos"]) {
    const receta = recetas.find((p) => p.id === recetaId);
    if (!receta) return;
    const newValue = !receta.pasos[paso];
    ...
    try {
      ...
      const res = await updateRecetaPasos(recetaId, { [dbKey]: newValue });
      if (!res.ok) {
  ...
  async function cambiarEstado(recetaId: string, estado: Estado) {
    try {
      const res = await updateRecetaEstado(recetaId, estado);
      if (!res.ok) { toast.error(res.error ?? "Error al cambiar estado"); return; }
      setRecetas((prev) =>
        prev.map((p) => (p.id === recetaId ? { ...p, estado } : p)),
      );
      if (detalle?.id === recetaId)
  ...
  async function eliminar(id: string) {
    try {
      const res = await deleteNuevaReceta(id);
      if (!res.ok) { toast.error(res.error ?? "Error al eliminar"); return; }
      setRecetas((prev) => prev.filter((p) => p.id !== id));
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex justify-end">
        <Button onClick={() => setShowNew(true)}>
          <Plus className="mr-1 h-4 w-4" /> Proponer receta
        </Button>
      </div>
