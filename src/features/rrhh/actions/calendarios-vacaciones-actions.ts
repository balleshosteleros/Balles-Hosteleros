...
/**
 * Calendarios activos de la empresa en curso, para elegir uno en el puesto.
 * Sin parámetro: la empresa sale del contexto de sesión, como en el resto de
 * pantallas de RRHH.
 */
export async function listCalendariosDeMiEmpresa(): Promise<{
  ok: boolean;
  data: { id: string; nombre: string; diasTotales: number; anio: number | null }[];
}> {
  try {
    ...
    console.error("[rrhh] listCalendariosDeMiEmpresa:", err);
    return { ok: true, data: [] };
  }
}

/**
 * Fija el calendario de vacaciones de un PUESTO y lo propaga a los empleados
 * que ya lo ocupan, igual que hace el departamento validador.
 *
 * Los empleados se localizan por el NOMBRE del puesto, porque `empleados.puesto`
 * es texto y no existe `empleados.puesto_id`.
 */
export async function setCalendarioVacacionesPuesto(input: {
  puestoId: string;
  calendarioId: string | null;
}): Promise<{ ok: boolean; empleadosActualizados?: number; error?: string }> {
  try {
    const { createAdminClient } = await import("@/lib/supabase/admin");
    const { requireAdminUser } = await import("@/features/rrhh/services/empleados-core");
    ...
    // Un calendario de otra empresa dejaría al empleado con días que no son
    // suyos, así que se comprueba antes de guardar.
    if (input.calendarioId) {
      const { data: cal } = await admin
        .from("rrhh_calendarios_vacaciones")
        .select("empresa_id")
        .eq("id", input.calendarioId)
        .maybeSingle();
      if ((cal?.empresa_id as string | null) !== empresaId) {
        return { ok: false, error: "El calendario no pertenece a esta empresa." };
      }
    }

    const { error: errP } = await admin
      .from("puestos")
      .update({ calendario_vacaciones_id: input.calendarioId })
      .eq("id", input.puestoId);
    if (errP) throw errP;
    ...
    const msg = err instanceof Error ? err.message : "Error desconocido";
    console.error("[rrhh] setCalendarioVacacionesPuesto:", msg);
    return { ok: false, error: msg };
  }
}
