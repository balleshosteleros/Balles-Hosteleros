...
import { Pencil, Search, Fingerprint, Check } from "lucide-react";
...
export function TiposFichajeSection({ empresaId }: { empresaId: string }) {
  const { items, loading, update } = useTiposFichaje(empresaId);
  ...
  const guardar = async () => {
    if (!editando || !form.nombre.trim() || !form.codigo.trim()) return;
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Estos son los tipos de fichaje que registra el sistema</p>
      </div>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Editar tipo de fichaje</DialogTitle></DialogHeader>
