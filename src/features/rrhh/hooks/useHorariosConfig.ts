...
export function useTiposAusencia() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listTiposAusencia();
  ...
  const create = useCallback(async (input: TipoAusenciaInput) => {
    const res = await createTipoAusencia(input);
    if (res.ok && res.data) {
  ...
  const update = useCallback(async (id: string, input: Partial<TipoAusenciaInput> & { orden?: number }) => {
...
export function useTiposFichaje() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listTiposFichaje();
      if (res.ok) setItems(res.data);
  ...
  const create = useCallback(async (input: TipoFichajeInput) => {
    const res = await createTipoFichaje(input);
    if (res.ok && res.data) {
      setItems((prev) => [...prev, res.data!]);
  ...
  const update = useCallback(async (id: string, input: Partial<TipoFichajeInput> & { orden?: number }) => {
